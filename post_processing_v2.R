...
sheets <- readxl::excel_sheets("C1148 Input files for Match 3 ISM_Prediction Intervals.xlsx")
dat <- c()

for(sh in sheets){
  tmp <- readxl::read_excel("C1148 Input files for Match 3 ISM_Prediction Intervals.xlsx", sheet = sh)
  tmp$scenario <- sh
  dat <- rbind(dat, tmp)
}
...
#figure 2
pdf(paste0("Figure_percentSampled_ISM_withCI.pdf"), paper="USr", width=10, height=7.25)
...
for(cv in c("0.5", "1.5", "3.0")){
  tmp <- read.csv(paste0("SimulationDecision_CV", cv, ".csv"))
  tmp$cv <- cv
  dat <- rbind(dat, tmp)
}

#figure 2
pdf(paste0("Figure_percentSampled_Discrete_withCI.pdf"), paper="USr", width=10, height=7.25)
par(omi=c(1,0.5, 0, 2.5), xpd=NA)
